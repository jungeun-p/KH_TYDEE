...
// 게시판 CRUD
			...
			if (res > 0) {
				...
				dispatch("tip.do?command=list", request, response);
			} else {
			...
			//팁게시판 댓글 관련 dto를 리스트로------------------------
			
			List<tip_Comm_dto>list = dao.selectListComment(tip_no,user_no);
			
			System.out.println(" list잘찍히는지 테스트 및 체크 : " + list);
			request.setAttribute("list", list);
...
// 팁 댓글 
			...
			if(user_id.equals("")||user_id==null) {
				jsResponse("로그인을 하셔야 합니다.", "tip_main.jsp", response);
			}else if( user_id !=null) {
				int tip_no = Integer.parseInt(request.getParameter("tip_no"));
				
				String tip_comm_content = request.getParameter("tip_comm_content");
				System.out.println(" 댓글 내용 : "+ tip_comm_content);
				tip_Comm_dto dto = new tip_Comm_dto(0,tip_no,user_no,tip_comm_content,null,null);
				
				int res = dao.insertReply(dto);
				
				if(res>0) {
					System.out.println("댓글 추가 성공");
					dispatch("tip.do?command=detail&tip_no="+tip_no , request, response);
				}else {
					System.out.println("댓글 추가 실패");
					jsResponse("댓글 추가 실패", "tip.do?command=detail&tip_no="+tip_no, response);
				}
			}
...
// 이미지 업로드
			...
			int tip_comm_no = Integer.parseInt(request.getParameter("tip_comm_no"));
			int tip_no = Integer.parseInt(request.getParameter("tip_no"));
			
			int res = dao.deleteReply(tip_comm_no);
			String user_id = request.getParameter("user_id");
			
			if(user_id.equals("")|| user_id==null ) {
				jsResponse("로그인을 하셔야 합니다.", "tip_index.jsp", response);
			}else if(user_id !=null) {
				if(res>0) {
					...
					jsResponse("댓글 삭제 실패!","tip.do?command=detail&tip_no=" + tip_no ,response);
				}
			}
