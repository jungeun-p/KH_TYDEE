...
<link
	...
	rel="stylesheet" />
<link rel="stylesheet" href="./css/tydee__eshop__cart.css" />
<!--font awsome-->
...
<body>
					...
					<c:forEach items="${list }" var="item">
						<div class="basket__item">
							<div class="item__image">
								<img
									src="./images/${empty item.item_image ? '1.jpg' : item.item_image }" />
							</div>
							<div class="item__title">
								<span>${item.item_title }</span> <span> <fmt:setLocale
										value="ko_KR" /> <fmt:formatNumber type="currency"
										value="${item.item_price}"></fmt:formatNumber>
								</span>
							</div>
							<div class="item__quantity" id="${item.basket_no}">
								<form action="basket.do" method="POST">
									<input type="hidden" name="basket_no"
										value="${item.basket_no }" /> <input type="hidden"
										name="item_no" value="${item.item_no }" />
									<div class="item__quantity_value">
										<input class="minus" type="button" value="-"
											onclick="quan_minus(${item.basket_no})" /> <input class="title__inner"
											type="number" name="item_quan" value="${item.item_quan }"
											min="1" required="required" /> <input class="plus" type="button"
											value="+" onclick="quan_plus(${item.basket_no})" />
									</div>
								</form>
								<div class="item__quantity_change">
									<input class="button" type="button" value="change"
										onclick="quan_change(${item.basket_no})" />
								</div>
							</div>
							<div class="buttons">
								<span onclick="quan_delete(${item.basket_no})"><i class="far fa-trash-alt"></i></span>
							</div>
						</div>
					</c:forEach>
				</c:otherwise>
	...
	<div class="total__price">
		<button class="goToList"
			onclick="location.href='item.do?command=list'">BACK</button>
		<span id="total_price">${totalPrice gt 50000 ? totalPrice : totalPrice + 3000 } WON</span>
		<button class="goOrder"
			onclick="location.href='order.do?command=ready&user_no=${loginuser.user_no}'">ORDER</button>
	</div>
